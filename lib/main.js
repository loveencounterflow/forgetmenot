...
(function() {
      ...
      return function*(resume) {
        ...
        for (i = 0, len = ref1.length; i < len; i++) {
          ...
          for (j = 0, len1 = paths.length; j < len1; j++) {
            ...
            target = files[path_checksum] != null ? files[path_checksum] : files[path_checksum] = _this._new_entry(me);
  ...
  this.set = function(me, name, value) {
    ...
    var cache, checksum, entry, json, key, ref1, timestamp;
    cache = me.cache;
    timestamp = this.DATE.as_timestamp();
    key = this.checksum_from_text(me, name);
    json = JSON.stringify(value);
    if (json === ((ref1 = cache[key]) != null ? ref1['value'] : void 0)) {
      return;
    }

    /* TAINT no need for checksum when doing string comparison */
    checksum = null;
    entry = cache[key] != null ? cache[key] : cache[key] = this._new_entry(me);
    Object.assign(entry, {
      path: name,
      value: json,
      checksum: checksum,
  ...
  this.get_entry = function(me, name, fallback) {
    var R, key;
    key = this.checksum_from_text(me, name);
  ...
  this.get = function(me, name, fallback) {
    var R;
    if ((R = this.get_entry(me, name, null)) === null) {
      if (fallback !== void 0) {
        return fallback;
      }
      throw new Error("no cache entry named " + (rpr(name)));
  ...
  this._file_entry_from_path = function(me, path) {
    ...
    if ((R = me['files'][path_checksum]) == null) {
